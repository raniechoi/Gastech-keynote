...
/**
 * Word 문서 생성 (바탕체, 14pt, 1줄간격)
 */
function generateDocxContent(data) {
  const { Paragraph, TextRun, AlignmentType, convertInchesToTwip } = require('docx');
  ...
  lines.forEach((line, index) => {
    ...
    // 날짜 (※로 시작)
      ...
      paragraphs.push(
        new Paragraph({
          ...
          spacing: {
            line: 240,
            after: 320  // 16pt (12pt * 16/12 = 320 twips)
          },
          runs: [
            new TextRun({
              text: trimmedLine,
    ...
    // "·" 항목
      ...
      const isLastDot = nextLine.startsWith('- ') || nextLine.startsWith('-');
      ...
      paragraphs.push(
        new Paragraph({
          ...
          spacing: {
            line: 240,
            after: isLastDot ? 320 : 240  // 마지막 "·"이면 16pt, 아니면 12pt
          },
          runs: [
            new TextRun({
              text: '  ' + content,
