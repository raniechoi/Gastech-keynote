...
function App() {
  ...
  // JSON 형식의 insightJson을 파싱해서 렌더링
  const renderInsight = () => {
    if (!result) return null;

    try {
      let insightData = result.insightJson;
      
      // insightJson이 문자열이면 파싱
      if (typeof insightData === 'string') {
        insightData = JSON.parse(insightData);
      }

      if (!insightData) {
        return <p className="error-text">정리 데이터를 찾을 수 없습니다</p>;
      }

      return (
        <div className="insight-content">
          {/* 제목 */}
          {insightData.speaker_name && insightData.speaker_title && (
          ...
          {/* 날짜 */}
          {insightData.date && insightData.time_end && (
            <p className="insight-date">
              * {insightData.date}, {insightData.time_start} {insightData.time_end}
            </p>
          )}

          {/* 인사이트 항목들 */}
          {insightData.insights && Array.isArray(insightData.insights) && (
            <div className="insights-list">
              {insightData.insights.map((insight, idx) => (
                <div key={idx} className="insight-item">
                  ...
                  {/* 부연설명 */}
                  {insight.subs && Array.isArray(insight.subs) && insight.subs.length > 0 && (
                    <div className="insight-subs">
                      {insight.subs.map((sub, subIdx) => (
                        <p key={subIdx} className="insight-sub">
                          · {sub}
                        </p>
                      ))}
                    </div>
          ...
          {/* 폴백: insight 필드가 있으면 (이전 형식 호환) */}
          {!insightData.insights && result.insight && (
      ...
      console.error('insightJson 파싱 에러:', err);
      return (
        <div className="error-text">
          <p>정리 데이터 렌더링 중 오류가 발생했습니다</p>
          <details>
